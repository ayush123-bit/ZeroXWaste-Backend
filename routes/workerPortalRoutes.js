...
// GET /api/workers-portal/dashboard — worker dashboard stats
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const workerUser = await User.findById(worker.userId).select('name email workerProfile picture');

    return res.status(200).json({
      status: 'success',
      data: {
        stats: { assigned, completed, pending, completionRate: assigned > 0 ? Math.round((completed/assigned)*100) : 0 },
        recentTasks,
        profile: workerUser,
      },
    });
...
// GET /api/workers-portal/tasks — all tasks for this worker
...
// POST /api/workers-portal/submit-proof/:reportId — upload cleaned proof
