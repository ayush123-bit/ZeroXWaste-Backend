...
const createReport = async (req, res) => {
  try {
    ...
    // All AI processing in background — response is instant
    setImmediate(async () => {
      try {
        const [priorityResult, classificationResult, recommendationResult] = await Promise.all([
          calculatePriorityScore({ imageUrl: uploadedImages[0]?.url, latitude: parseFloat(latitude), longitude: parseFloat(longitude), address: address || '', description }),
        ...
        if (user?.userId) {
          ...
          if (priorityResult.priorityLevel === 'High') {
            await awardPoints(user.userId, 'HIGH_PRIORITY_REPORT');
          }
        }

        // Broadcast to all admins if high priority
        if (priorityResult.priorityLevel === 'High') {
          broadcast({
            type: 'NEW_HIGH_PRIORITY',
            title: 'New High Priority Report',
            message: `${category} waste reported near ${address || 'unknown location'}`,
            reportId: report._id,
...
// GET /api/reports/heatmap — returns lightweight coordinate + score data for map
const getHeatmapData = async (req, res) => {
  try {
    const reports = await Report.find({ 'location.coordinates': { $exists: true } })
      .select('location priorityScore priorityLevel category status clusterSize')
      .limit(500);

    const points = reports.map(r => ({
      lat: r.location.coordinates[1],
      lng: r.location.coordinates[0],
      weight: (r.priorityScore || 30) / 100,
      priorityLevel: r.priorityLevel,
      category: r.category,
      status: r.status,
      clusterSize: r.clusterSize || 1,
    }));
...
const getReportStats = async (req, res) => {
  try {
    const cache = require('./cacheService'); // lazy require to avoid circular
    const CACHE_KEY = 'stats:main';
    // Note: import cacheService properly at top in real file
    const [overall, byCategory, byPriority, trend] = await Promise.all([
      Report.aggregate([{ $group: { _id: null, totalReports: { $sum: 1 }, pendingReports: { $sum: { $cond: [{ $eq: ['$status', 'pending'] }, 1, 0] } }, inProgressReports: { $sum: { $cond: [{ $eq: ['$status', 'in-progress'] }, 1, 0] } }, resolvedReports: { $sum: { $cond: [{ $eq: ['$status', 'resolved'] }, 1, 0] } }, avgPriorityScore: { $avg: '$priorityScore' } } }]),
      Report.aggregate([{ $group: { _id: '$category', count: { $sum: 1 } } }]),
      Report.aggregate([{ $group: { _id: '$priorityLevel', count: { $sum: 1 }, avgScore: { $avg: '$priorityScore' } } }, { $sort: { avgScore: -1 } }]),
      Report.aggregate([
    ...
    return res.status(200).json({
      status: 'success',
      data: { overall: overall[0] || { totalReports: 0, pendingReports: 0, inProgressReports: 0, resolvedReports: 0, avgPriorityScore: 0 }, byCategory, byPriority, trend: trend.reverse() },
    });
  } catch (error) {
...
// PATCH /api/reports/:id/assign — Admin assigns to worker
const assignWorker = async (req, res) => {
  try {
    const { workerId, workerName } = req.body;
    const report = await Report.findByIdAndUpdate(
      req.params.id,
      { assignedWorker: { workerId, workerName, assignedAt: new Date() }, status: 'in-progress' },
      { new: true }
    );
    if (!report) return res.status(404).json({ status: 'error', message: 'Report not found' });
